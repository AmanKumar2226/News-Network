  ...
  function One() {
    ...
    // https://newsapi.org/v2/top-headlines?country=in&category=general&apiKey=01ef4ed41f254070bcf6f7ba22ad5d3c
  ...
  {news.slice(-2).map((newsObj, index) => {
    return (
      <div className='relative w-1/2' key={index}>
        <div className=''>
          <a href={newsObj.url} target='_blank'>
          <img className='h-[456px] w-[40vh] rounded-xl' src={`http://localhost:8080/${newsObj.image}`} alt="" />
          </a>
        </div>
                  ...
                  <div className="w-[50%] h-full bg-white dark:bg-[#2A2C38] dark:text-white text-black rounded-xl">
                    <div className="rounded-xl h-[50vh] flex justify-center items-end bg-no-repeat bg-cover bg-center" style={{ backgroundImage: `url(${dataObj.urlToImage})` }}>
                      ...
                      <div className="flex flex-col bg-[rgba(255,255,255,0.75)] dark:bg-[rgba(42,44,56,0.75)]  dark:border-none gap-4  w-[95%] p-5 rounded-xl mb-6">
                        <a href={dataObj.url} target='_blank'><p className="line-clamp-1 text-xl font-semibold">{dataObj.title}</p></a>
