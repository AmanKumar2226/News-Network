...
const CategoryDialog = ({ isOpen, onClose, className }) => {
  return (
    <div className={`fixed z-[999999] top-0 right-0 h-full w-full md:w-1/2 bg-white p-5 transition-transform ${isOpen ? className : ''} ${isOpen ? 'block' : 'hidden'}`}>
        ...
        <button onClick={onClose} className="mb-4 bg-[#F5F5F5] p-2 rounded-xl w-[50px] text-xl font-bold">X</button>
        </div>
      
      <nav className="rounded-xl w-full mx-2 bg-[#F5F5F5] flex flex-col gap-6 cursor-pointer justify-center py-5">
        {[
